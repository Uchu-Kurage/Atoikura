...
- **Daily threat wave (the expense minigame):** expense registration is instant (`settleThreat` records the threat and applies HP damage immediately, no reward). The invader minigame is **batched to the next day, one calendar day per wave**: `getPendingWave` returns the *oldest* unprocessed day that has expenses (from `currentMonth.lastWaveDate` up to yesterday, today excluded) rather than merging days, so a missed day just waits in the queue and can be fought later. `startWave` plays one invader per expense on that day (drawn with its category icon), rolls a single data-chip reward — gated on that day's total staying within the daily budget pace (`baseHp / daysInMonth`) and scaled by kill ratio (`0.20 + 0.30·ratio`) — then advances `lastWaveDate` to the day *after* the fought day (`nextYmd`). Since each acknowledgement re-enters `go('home')`, remaining missed days chain through automatically. `lastWaveDate` is backfilled to today on first run so existing saves aren't ambushed by a backlog. When `character.minigameOn` is false (or `prefers-reduced-motion`), each wave auto-resolves with the base 20% roll and no game UI. The minigame never changes money or HP.
