...
- **Gadget mods (minigame upgrades):** materials 🔩 fund permanent minigame upgrades defined in `GADGETS` (`fire`/`spread`/`beam`/`slow`), stored as levels in `state.gadgets`. The 撃退 (`waves`) screen renders them (`renderGadgets`, alongside the calendar and the debug panel) and `upgradeGadget(id)` spends materials to level up. `getGadgetEffects(state)` resolves the active levels into game modifiers (auto-fire interval, bullets per volley, hit radius, invader speed multiplier) which `startInvaderGame` applies. The minigame has **no time limit** — it clears when every threat is destroyed, and ends if the block breaches the turret line (survivors count as not-killed). Because wave materials are frugality-gated, the upgrade loop is powered by saving, not spending.
