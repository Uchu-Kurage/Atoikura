...
- **State:** One JSON object persisted under `localStorage` key `SK = 'kakebo_quest_v1'`. Read/write via `load()` / `save(state)`. Shape:
  ...
  { character:   { name, avatar, monthlyBudget, categories[], lifetimeSaved, fixedCosts[], carryover, unbudgetedAsTemp, minigameOn },
    currentMonth:{ year, month, baseHp, currentHp, expenses[], tempExpenses[], checkpoints[], status, processedWaveDays[] },
    inventory:   { materials, unlockedChips[] },
    gadgets:     { fire, spread, beam, slow },   // minigame mod levels
    baseLevel,
    history:     [ …past currentMonth snapshots… ] }
  ```
- **Game-loop gating happens inside `go('home')`:** before showing home it calls `checkMonthEnd(state)` (rolls the month over if the calendar month advanced, computing bonuses and pushing the old month to `history`), then `getPendingCp(state)` (an un-acknowledged past week). Either can redirect to the `mend`/`cp` result screen instead of home. This means month-rollover and checkpoint logic are **only** triggered by navigating home — not on a timer.
- **Threat-wave calendar (the expense minigame):** expense registration is instant (`settleThreat` records the threat and applies HP damage immediately, no reward). The invader minigame is **player-selected from a calendar**, not auto-triggered. The `waves` screen (`renderWaves`) draws the current month grid; each **past** day (today excluded — today is still accumulating) that has expenses is either *pending* (tappable, gold) or *processed* (✓). Tapping a pending day calls `playWaveDay(dayStr)`, which plays one invader per expense on that day (drawn with its category icon), then grants a **frugality-scaled** reward and records the day in `currentMonth.processedWaveDays[]`. `frugality = clamp(1.5 − r, 0, 1)` where `r = dayTotal / (baseHp/daysInMonth)` (r≤0.5→1.0, r=1.0→0.5, r≥1.5→0). The data-chip chance is `0.5 · frugality · (0.5 + 0.5·killRatio)` and materials are `round(40 · frugality · (0.4 + 0.6·killRatio))`. Frugality is a **multiplicative gate**, so an overspending day yields nothing no matter how well it's played — savings, not skill, drives the reward. Minigame difficulty also scales with `r` (a bigger overspend sweeps faster), passed into `startInvaderGame(icons, label, onDone, ratio)`. `pendingWaveDays(cm)` lists the selectable days; home shows a `#hWaveEntry` banner with the count. `ensureWaveState` backfills `processedWaveDays` from the legacy `lastWaveDate` boundary (days before it marked processed; new saves anchor to today) so existing saves aren't surfaced as a backlog. When `character.minigameOn` is false (or `prefers-reduced-motion`), tapping a day auto-resolves with the base 20% roll and no game UI. The minigame never changes money or HP.
- **Narrative framing (minigame layer):** in UI copy, an expense **charges energy** (エネルギーをチャージ) and the next day's minigame spends that charge to **repel** (撃退) the day's threats — the `waves` tab is the 撃退 (repel) tab. The underlying HP/budget model is unchanged (spending still consumes the core). Lifetime badges/data-chip lore keep the broader survival framing.
- **Gadget mods (minigame upgrades):** materials 🔩 fund permanent minigame upgrades defined in `GADGETS` (`fire`/`spread`/`beam`/`slow`), stored as levels in `state.gadgets`. The 拠点 (`base`) screen renders them (`renderGadgets`) and `upgradeGadget(id)` spends materials to level up. Each gadget unlocks at a required base level (`GADGETS[].min`, 1/2/3/4), so raising the 拠点 grade widens the range of available mods; locked rows show 拠点Lv.N で解放. `getGadgetEffects(state)` resolves the active levels into game modifiers (auto-fire interval, bullets per volley, hit radius, invader speed multiplier) which `startInvaderGame` applies. The minigame has **no time limit** — it clears when every threat is destroyed, and ends in failure if the block breaches the turret line. A breach (`finish(true)` → `onDone(ratio, breached)`) **forfeits the whole reward** for that day (no materials, no chip), though the day is still marked processed; otherwise the reward is frugality × kill-ratio as above. Because wave materials are frugality-gated, the upgrade loop is powered by saving, not spending.
- **Minigame debug mode:** `isDebug()` (true when the URL carries a `debug` token, e.g. `…/#debug`, or `localStorage.kakebo_debug === '1'`) reveals a debug panel on the `waves` screen — and always shows the home `#hWaveEntry` banner — so the invader minigame can be launched on demand with an arbitrary threat count, with no real past-day expenses required. `debugRunMiniGame(n, cb)` calls `startInvaderGame` directly and **never mutates save state** (no HP/chip/`processedWaveDays` changes). Console helpers: `debugMiniGame(n)` launches from any screen; `atoikuraDebug(true|false)` toggles the persisted flag and re-renders.
