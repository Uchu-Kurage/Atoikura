...
- **Gadget mods (minigame upgrades):** materials 🔩 fund permanent minigame upgrades defined in `GADGETS` (`fire`/`spread`/`beam`/`slow`), stored as levels in `state.gadgets`. The base ("部屋") screen renders them (`renderGadgets`) and `upgradeGadget(id)` spends materials to level up. `getGadgetEffects(state)` resolves the active levels into game modifiers (auto-fire interval, bullets per volley, hit radius, invader speed multiplier, timer bonus) which `startInvaderGame` applies. Because wave materials are frugality-gated, the upgrade loop is powered by saving, not spending.
