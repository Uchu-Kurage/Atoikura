...
- **Narrative framing (minigame layer):** in UI copy, an expense **charges energy** (エネルギーをチャージ) and the next day's minigame **releases** it (エネルギー放出) — the `waves` tab is the 放出 (release) tab. The underlying HP/budget model is unchanged (spending still consumes the core); only the minigame-layer wording uses charge/release instead of threat/repel. Lifetime badges/data-chip lore keep the broader survival framing.
- **Gadget mods (minigame upgrades):** materials 🔩 fund permanent minigame upgrades defined in `GADGETS` (`fire`/`spread`/`beam`/`slow`), stored as levels in `state.gadgets`. The 放出 (`waves`) screen renders them (`renderGadgets`, alongside the calendar and the debug panel) and `upgradeGadget(id)` spends materials to level up. `getGadgetEffects(state)` resolves the active levels into game modifiers (auto-fire interval, bullets per volley, hit radius, invader speed multiplier, timer bonus) which `startInvaderGame` applies. Because wave materials are frugality-gated, the upgrade loop is powered by saving, not spending.
